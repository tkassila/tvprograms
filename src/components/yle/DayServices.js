  ...
  getTableHeadersAndTableRowsAfterServices = () =>
  {
    ...
    if (services != null)
    {
      ...
      let tmp_channels = services.map((s2, i) => {
        return <YleService data={s2} selectedcategory={this.props.selectedcategory} 
  ...
  render(props, state)
  {
    ...
    {      
        ...
        headers = state.schedules.map((s, i) => { 
          return s.service.title.fi; 
        });
        let tmp_channels = state.schedules.map((s2, k) => {
          return <YleChannel id={k} data={s2.service} schedules={s2} selectedcategory={this.props.selectedcategory} 
                  yleapiparams={this.props.yleapiparams} progsource={this.props.progsource} 
