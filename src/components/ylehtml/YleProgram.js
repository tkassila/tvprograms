		...
		return (			
			<div>
				<div onClick={onClickDisplay} onKeyDown={enterKeyDown} > 
					<h4 lang="fi" tabIndex="0"  >{getProgramSymbol(props)} {props.data.startTime ? 
                    props.data.startTime +" " : ''}  
					{ (props.showSearch 
                        && props.data.titleindex != undefined
                    ? getPOfTitleIndex(props) : (props.data.name ? props.data.name +" "  : '') )
                    } {props.data.captiontext ? props.data.captiontext : ''}</h4>
					{props.bShowDesciption && <p style={{ "fontWeight": 'normal', "text-align": "left" }}
                    lang="fi" tabIndex="0">{props.data.description ? 
