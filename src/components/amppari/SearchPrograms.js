...
export default class SearchPrograms extends Component {
	...
	{
		if(Config.bDebug) 				
			console.log("componentDidMount SearchPrograms");
	...
	componentWillReceiveProps(nextProps) 
	{
	  ...
	  if (!this._mounted)
	  	  return;
	  if (nextProps.disabled != undefined && nextProps.disabled != null && nextProps.disabled != this.props.disabled)
    ...
    listenerStoreChange2 = (storestate) =>
    {
		...
		if (programtypeitems == undefined || programtypeitems == null)
			return;
		if (programtypeitems == this.state.programtypeitems)
			return;
		...
		*/
		...
		console.log(items);
		*/
		this.store.setState( { channels: items, bSearchButtonClicked: bSearchButtonClicked,
			textSearch: this.textSearch, bDisplayAllDescriptions: bDisplayAllDescriptions });
    }
	...
	searchTextFromChannelsClicked = (event) => 
	{
		...
		console.log(items);	
		this.store.setState( { channels: items, bSearchButtonClicked: bSearchButtonClicked,
			textSearch: this.textSearch, bDisplayAllDescriptions: true });
	...
	makeSearchChange = (change) =>
	{
		...
		if (/* bSearch && */ (change === undefined || change === null 
			|| change.toString().trim().length == 0 ))
		{
			this.textSearch = null;
			this.setState({ textSearch: null, bSearchButtonClicked: false});
			this.store.setState( { channels: this.programtypeitems, bSearchButtonClicked: false,
					...
					// this.state.channels.forEach(cha, i => {
						...
						*/
						...
						// cha.channelprograms.forEach(pr, i => {
							progfounded = false;
							prcopy = null;
							index = (pr.title ? pr.title.toString().toUpperCase().indexOf(uppersearchText) : -1);
							if (index != -1)
							{
								if (prcopy == null)
								{
							...
							index = (pr.description ? pr.description.toString().toUpperCase().indexOf(uppersearchText) : -1);
							if (index != -1)
							{
								if (prcopy == null)
								{
