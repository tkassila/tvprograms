...
export default class Amppari extends Component {
	...
	ilmaisetchannelnames = ['Yle TV1','Yle TV2','MTV3','Nelonen','Yle Teema Fem',
		'Sub','TV Viisi','Liv','Jim','Kutonen','TLC','Fox','Ava','Hero','alfatv','frii',
		'National Geoprafic'];
	...
	removelisteners = () =>
	{
		...
		if (this.removerfunctionProgramType)
			this.removerfunctionProgramType();
		if (this.removerfunctionSearchProgram)
			this.removerfunctionSearchProgram();
		if (this.removerfunctionChannelType)
			this.removerfunctionChannelType();
	}

	setRemoverFunction = (func, fname) =>
	{
		if (fname === 'ProgramTypes')
			this.removerfunctionProgramType = func;
		else
		if (fname === 'SearchPrograms')
			this.removerfunctionSearchProgram = func;
		else
		if (fname === 'ChannelTypes')
			this.removerfunctionChannelType = func;
	}
	...
	listenerStoreChange2 = (storestate) =>
    {
		...
		if (store_selectedtyyppi && store_selectedtyyppi != this.state.selectedtyyppi)
		{
		...
		*/
        ...
        this.setState({ channels: channels, textSearch: textSearch,
	...
	getNextJsonArrayAfterLabel(json, labelitem)
	{
		...
		*/
		if (labelitem == undefined || labelitem == null)
			return null;
		if (json == undefined || json == null || json.length == 0)
			return null;
		let iCount = 0;
		let found = false, founded = false;
	...
	getChannelsAndProgramsFromJsonArray = (json) =>
	{
		...
		let tv = null, programming = null, f = null, channel = null, programs = null;
		let i, j, max = json.length, maxj, data = null;
		let searchtext = 'tv', foundSearchText = false;

		for(i = 0; i < max; i++)
		{
			if (!foundSearchText && typeof json[i] === 'string' && json[i] === searchtext)
				...
				if (searchtext === 'tv')
				{
					...
					for(j = 0; j < maxj; j++)
					{
						...
						*/
						...
						let k = 0, maxk = programs.length, jsonprogram, progdata, jsonprograms = [];
						for(k = 0; k < maxk; k++)
						{
							jsonprogram = {};
							...
							jsonprogram.timestamp = this.getNextJsonArrayAfterLabel(progdata, 'timestamp');
							jsonprogram.title = this.getNextJsonArrayAfterLabel(progdata, 'title');
							jsonprogram.description = this.getNextJsonArrayAfterLabel(progdata, 'description');
							jsonprogram.channel = this.getNextJsonArrayAfterLabel(progdata, 'channel');
							jsonprogram.movie = this.getNextJsonArrayAfterLabel(progdata, 'movie');
							jsonprogram.sport = this.getNextJsonArrayAfterLabel(progdata, 'sport');
	...
	fetchHtmlAmppariChannels = async (dayparameter) =>
	{
		...
		this.setState({ bUnderFetch: true, channels: [], selectedchannelindex: 0,
			currentChannelSetIndex: 0, errmsg: null, });
...
//		.then(str => new window.DOMParser().parseFromString(str, "text/xml"))
				...
				if (index > -1)
				{
					...
					if (indexEnd > -1)
					{
						...
						*/
						...
						// this.channels = data;
						this.setState({ fetcheditems: this.fetcheditems,
							channels: [],
						...
						//this.filterWhenUIControlsHasBeenChanged(filtercalled.CHANNELTYPE);
						*/						
						this.setState({ /* fetcheditems: this.fetcheditems, */ channels: this.channels,
							bUnderFetch: false, selectedsuodattimet: 'kaikki',
								selectedtyyppi: 'kaikki', selectedTyyppiinindex: 0,
								selectedsuodatinindex: 0, bSearchButtonClicked: false,
								selectedchannelindex: 0 });
	...
	{		  
		...
		Array.from(data).forEach(pr => {
			...
			if (channel !== null)
			{
				if (channel.channelprograms == undefined)
					channel.channelprograms = [];
	...
	{
		let ret = null;
		if (strprorams === null || strprorams === undefined)
			return null;
		let indStart = 0;
		let searchResultChannel = this.getJsonLabel('channel', strprograms, indStart);
		...
		{						
			...
			searchResult = this.getJsonLabel('id', strprograms, indStart, indSecondChannel);
			if (searchResult !== null)
			{								
				indStart = searchResult.indEnd;	
				searchresult = this.getJsonNextValue(strprograms, indStart, indSecondChannel);
				if (searchResult !== null)
				{									
					let channelid = searchResult.value;
					indStart = searchResult.indEnd;
					searchResult = this.getJsonLabel('name', strprograms, indStart, indSecondChannel);
					if (searchResult !== null)
					{														
						indStart = searchResult.indEnd;	
						searchresult = this.getJsonNextValue(strprograms, indStart, indSecondChannel);
						...
						searchResult = this.getJsonLabel('slug', strprograms, indStart, indSecondChannel);
						if (searchResult !== null)
						{														
							indStart = searchResult.indEnd;	
							searchresult = this.getJsonNextValue(strprograms, indStart, indSecondChannel);
							...
							searchResult = this.getJsonLabel('pay', strprograms, indStart, indSecondChannel);
							if (searchResult !== null)
							{														
								indStart = searchResult.indEnd;	
								searchresult = this.getJsonNextValue(strprograms, indStart, indSecondChannel);
								...
								firstIndex = strprorams.indexOf(this.search_start_program, indStart);
								firstIndexEnd = strprorams.indexOf(search_end_program);
								strprogram = null;
								jsonprog =  null;
								iCount = 1;
						
								while(firstIndex !== -1 && firstIndexEnd !== -1 && 
									(indSecondChannel == -1 || (indSecondChannel != -1 && firstIndexEnd < indSecondChannel)))
								{
									...
									if (jsonprog != null)
									{
										...
										break;
									firstIndex = strprorams.indexOf(this.search_start_program, firstIndexEnd);
	...
	{
		if (strprogram === null || strprogram === undefined)
			return null;			
		...
		if (indId > -1)
		{
			...
			searchresult = this.getJsonLabelAndValue("id", strprogram, indStart, indSecondChannel);
			if (searchresult != null)
			{
				...
				indStart = searchresult.indEnd;
				searchresult = this.getJsonLabelAndValue("timestamp", strprogram, indStart, indSecondChannel);
				if (searchresult != null)
				{
				...
				console.log(strjson);			
					indStart = searchresult.indEnd;
					searchresult = this.getJsonLabelAndValue("title", strprogram, indStart, indSecondChannel);
					if (searchresult != null)
					{
						strjson = strjson +",\"" +searchresult.name +"\":" +searchresult.value;
					...
					console.log(strjson);			
						indStart = searchresult.indEnd;
						searchresult = this.getJsonLabelAndValue("description", strprogram, indStart, indSecondChannel);
						if (searchresult != null)
						{
							strjson = strjson +",\"" +searchresult.name +"\":" +searchresult.value;
							indStart = searchresult.indEnd;
						...
						console.log(strjson);			
							searchresult = this.getJsonLabelAndValue("channel", strprogram, indStart, indSecondChannel);
							if (searchresult != null)
							{
								strjson = strjson +",\"" +searchresult.name +"\":" +searchresult.value;
							...
							console.log(strjson);			
								indStart = searchresult.indEnd;
								searchresult = this.getJsonLabelAndValue("movie", strprogram, indStart, indSecondChannel);
								if (searchresult != null)
								{
									strjson = strjson +",\"" +searchresult.name +"\":" +searchresult.value;
								...
								console.log(strjson);			
									indStart = searchresult.indEnd;
									searchresult = this.getJsonLabelAndValue("sport", strprogram, indStart, indSecondChannel);
									if (searchresult != null)
									{
										strjson = strjson +",\"" +searchresult.name +"\":" +searchresult.value;
										strjson = strjson +" }"
	...
	getJsonLabelAndValue = (name, strprogram, indStart, indSecondChannel) => 
	{
		...
		let searchresult = this.getJsonNextLabel(strprogram, indStart, indSecondChannel);
		if (searchresult != null)
		{
			let indStart2 = searchresult.indEnd;
			let name2 = searchresult.name;
			searchresult = this.getJsonNextValue(strprogram, indStart2, indSecondChannel);
			if (searchresult != null)
			{
	...
	getJsonNextValue = (strprogram, indStart, indSecondChannel) =>
	{
		...
		if (ind > -1)
		{
			...
			let indEnd = -1;
			if (bStartWithStrike)
				indEnd = strprogram.indexOf('"', ind+iAdd);
			else
				indEnd = strprogram.indexOf(search, ind+iAdd);
			...
			{
				if (bStartWithStrike)
					searchresult.indEnd = indEnd+1;
				else
					searchresult.indEnd = indEnd;
				if (bStartWithStrike)
					searchresult.value = strprogram.substring(ind+1, indEnd+1);
				else
					searchresult.value = strprogram.substring(ind+1, indEnd);
	...
	*/
	...
	//	console.log("showOneChannel");
		...
		else
		{
			const st = this.store;
			const storestate = st.getState();
			this.channels = storestate.channels;-1
				...
				if (this.state.showChannelsAtSameTime < all_channels.length)
				{
					...
					if (!bAllChannels)
					{
						...
						channels = newChannelSet.map((jsonchannel, i) => {
							return <AmppariChannel id={i} data={jsonchannel}
									showSearch={this.state.bSearchButtonClicked
...
//				console.log(filteredchannels);
	...
	filterAfterProgramType = (change) =>
	{
		...
		let mychange = null;
		if (change == undefined)
			mychange = this.state.selectedtyyppi;
		else
		{
			ind = change.ind;
					...
					// this.state.channels.forEach(cha, i => {
						...
						*/
						...
						// cha.channelprograms.forEach(pr, i => {
							...
							prcopy = {};
							Object.assign(prcopy, pr);
							index = (prcopy.title ? prcopy.title.toString().toUpperCase().indexOf(uppersearchText): -1);
							if (index != -1)
							{
								prcopy.titleindex = index;
								/*
		...
		{  // selected={child.title.fi===state.selectedcategory.title.fi} 
			...
			if (channels != null)
				tabletds = channels.map((child, i) => {
					return <td  tabIndex="0" id={'tablecol' +i} lang="fi" 
		...
		return (
			...
			<div class={style.cardHeader} onKeyUp={this.altPlusKeyUpProgramHeader}>
				...
				<Card>
					...
					<space>          </space>
					<SearchPrograms disabled={state.fetcheditems == null 
						|| state.fetcheditems.length == 0}
				...
				<div>
					...
					Ei ohjelmatietoja haettu tai tekstihakutulosta (tyhjää hakutekstikenttä)</div> }
							...
							<table id="programtable" style={"width:100%; " +tableBorderStyle}
							ref={this.tablCntl}>
							<tbody>
							<tr>{tableheaders}</tr>
							<tr>{tabletds}</tr> 		
