...
//    const [displayAllDescriptions, setDisplayAllDescriptions] = useState(props.displayAllDescriptions);
   ...
   // console.log("onClickDisplay");
    ...
    return (           
            <div>
                <div >
                <div tabIndex="0" lang="fi" onClick={onClickDisplay} onKeyDown={enterKeyDown}>                  
                <p><b>{props.showSearch 
                        && props.data.titleindex != undefined
                    ? <div >{getProgramSymbol(props)} {getTime(props.data.timestamp)}<space> </space>  
                     {getPOfTitleIndex(props)}</div> 
                    : <div>{getProgramSymbol(props)} {getTime(props.data.timestamp)}<space> </space> 
                     {props.data.title.toString()}</div>}</b></p>
                    ...
                    <Fragment><p>{ props.showSearch 
                        ...
                        ? props.getPOfIndex(props.data.descriptionindex, 
                            props.data.description.toString(), props.themevalue) 
