...
export default class ProgramTypes extends Component {
	...
	{
		if(Config.bDebug) 				
			console.log("componentDidMount ProgramTypes");
	...
	componentWillReceiveProps(nextProps) 
	{
	  ...
	  if (!this._mounted)
	  	return;
	  if (nextProps.disabled != undefined && nextProps.disabled != this.props.disabled)
    ...
    listenerStoreChange2 = (storestate) =>
    {
		...
		if (fetchitems == this.state.channeltypeitems)
			return;
					  ...
					  <select tabIndex="0" 
							...
							onChange={(e)=>{
								...
								changed.selected = selected;
									...
									this.store.setState({programtypeitems: items, selectedtyyppi: selected});
								}}>
					        {selecttyyppi_items}
				    </select>
					</div>					  
