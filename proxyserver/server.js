...
{
  let regexPattern = /<time\s{1,}itemprop="startDate"\s{1,}datetime="([^".]*)">(\d*\.\d*)*<\/time>/gi;
  let regexPattern2 = /<time\s{1,}itemprop="endDate"\s{1,}datetime="([^".]*)">(\d*\.\d*)*<\/time>/gi;
  let regexPattern3 = /<meta itemprop="duration" content="([^".]*)">/gi;
  // let regexPattern = /<h2/;
  let regex = new RegExp(regexPattern);
  // let result = textHtml.match(regex);
  let result, result2 = null;
  // let max = result.length;
  let ind = 0;
  let startDateStr = null, startTimeStr = null;
  let endDateStr = null, endTimeStr = null;
  let spanStr = progStr;
  let jsonProgItem = {};

  jsonProgItem.accessibilityfeature = false;

  let regexPattern1 = /<div\s{1,}class="schedule-card__header"\s{1,}tabIndex="0">/gi;
  ...
  if (result1)
  {
    ...
    while (result4)
    {
      ...
      if (itemPropName == 'duration')
        durationStr = itemPropContent;
      if (itemPropName == 'accessibilityFeature' && itemPropContent == 'captions')
    ...
    if (indRegex1 > -1)
    {
      ...
      if (result)
      {
        ...
        if (result5)
        {
          ...
          if (ind5 > -1)
          {
            ...
            let regex6 = new RegExp(/<span\s{1,}itemprop="name">([^<.]*)<\/span>/gi);
      ...
      let regex6b = new RegExp(/<li\s+class="schedule-card\s+schedule-card([^\s.]*\s)/gi);
    ...
    if (result7b)
    {
      ...
      let regex9 = new RegExp(/<span\s+class="schedule-card__duration">([^<\n.]*)</gi);
...
function parseOneYleChannelFromHtml (blog) 
{
  ...
  let regexPattern = /aria-label="([^".]*)"/gi;
    ...
    let channeltitle = '';
    if (result)
      channeltitle = result[1];

      let ind = 0;
      const searchstr = '<li class="schedule-card';
      let liInd = blog.indexOf(searchstr, ind);
      let progStr = null, indEndLi = -1;
      let progitems = [], bEnd = false, progItem = null;
      let i = 1;
...
{
  if (textHtml === undefined || textHtml === null)
    return "";
  ...
  */
  ...
  let prev_matchStart = -1, prev_matchEnd = 0;
  let next_matchStart = -1, next_matchEnd = -1;		
  let before = null, found = null, after = null, blog = null;

  let foundedRegex = false, mathlength;
  let mathlength2, next_matchEnd2;
  let i = 1, channel = null, channels = [];
...
//    console.log("blog");
 ...
 //   });
    ...
    */
    ...
    if (next_matchEnd)
      prev_matchEnd = next_matchEnd;
    else
      prev_matchEnd = matchEnd;
...
app.get('/telkku/:channel', (req, res) => {
    const cmd = 'curl "https://telkussa.fi/RSS/Channel/' +req.params.channel +'"';
    // res.send(cmd);
...
app.get('/ampparissa/:rest', (req, res) => {
    ...
    console.log(query);
    const cmd = 'curl "https://www.ampparit.com/' +req.params.rest +"?" +query.toString() +'"';
    // res.send(cmd);
    execShellCommandXmlData(cmd, res)
  })
